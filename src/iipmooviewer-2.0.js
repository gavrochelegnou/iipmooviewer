...
var IIPMooViewer = new Class({
    ...
    initialize: function(main_id, options) {
        ...
        if (typeOf(options.image) == 'array') {
            for (i = 0; i < options.image.length; i++) {
                if (i === 0) {
                    this.opacityLayers.push(1);
                } else {
                    this.opacityLayers.push(0);
                }
                this.images[i] = {src: options.image[i], sds: "0,90", cnt: (this.viewport && this.viewport.contrast != null) ? this.viewport.contrast : null};
            }
        }
        else
            this.images = [{src: options.image, sds: "0,90", cnt: (this.viewport && this.viewport.contrast != null) ? this.viewport.contrast : null}];
        ...
        if ((typeof(Navigation) === "function")) {
            this.navigation = new Navigation(this,{showNavWindow: options.showNavWindow,
        ...
        if (Browser.firefox)
            this.CSSprefix = '-moz-';
        else if (Browser.chrome || Browser.safari || Browser.Platform.ios)
    ...
    loadGrid: function() {
        ...
        if (this.wid < view.w)
            xoffset -= (view.w - this.wid) / 2;

        var yoffset = Math.floor(view.y % this.tileSize.h);
        if (this.hei < view.h)
            yoffset -= (view.h - this.hei) / 2;
        ...
        for (var m = 0; m < ntiles; m++) {
            ...
            if (this.tiles.contains(k)) {
                ...
                if (this.navigation)
                    this.navigation.refreshLoadBar(this.nTilesLoaded, this.nTilesToLoad);
                if (this.nTilesLoaded >= this.nTilesToLoad)
                    this.canvas.setStyle('cursor', null);
                continue;
            }
            ...
            for (n = 0; n < this.images.length; n++) {
                ...
                tile.addEvents({
                    'load': function(tile, id) {
                        if (this.effects)
                            tile.setStyle('opacity', this.opacityLayers[tile.layer]);                       
        ...
        switch (e.code) {
            case 37: // left
                ...
                if (IIPMooViewer.sync)
                    IIPMooViewer.windows(this).invoke('nudge', -d, 0);
                event.preventDefault(); // Prevent default only for navigational keys
                break;
            case 38: // up
                this.nudge(0, -d);
                if (IIPMooViewer.sync)
                    IIPMooViewer.windows(this).invoke('nudge', 0, -d);
            ...
            case 67: // For control-c, show our current view location
                if (e.control)
                    prompt("URL of current view:", window.location.href.split("#")[0] + '#' +
    ...
    toggleFullScreen: function() {
        ...
        if (!this.fullscreen.isFullscreen) {
            ...
            if (this.fullscreen.enter && !this.fullscreen.isFullscreen)
            ...
            if (this.fullscreen.exit && this.fullscreen.isFullscreen)
    ...
    getView: function() {
        ...
        if (Math.abs(this.view.rotation % 180) == 90) {
            ...
            if (x < 0)
                x = 0;  // Make sure we don't have -ve values
            if (y < 0)
                y = 0;
            w = this.view.h;
            h = this.view.w;
    ...
    createWindows: function() {
        ...
        if (this.click) {
            ...
            this.touch.addEvents({
                start: function(e) {
                    _this.canvas.removeEvents('mouseup');
                },
                complete: function(e) {
                    _this.canvas.addEvent('mouseup', fn);
                }
            });
        }
        ...
        if (!(Browser.Platform.ios || Browser.Platform.android)) {
            ...
            if (Browser.ie8 || Browser.ie7)
                tip_list = 'img.logo, div.toolbar'; // IE8 bug which triggers window resize
            new Tips(tip_list, {
    ...
    transformCoords: function(x, y) {
        ...
        // Return raw pixel values
            ...
            return Math.round(x * this.wid) + 'px, ' + Math.round(y * this.hei) + 'px';
    },
...
IIPMooViewer.windows = function(s) {
    if (!this.sync || !this.sync.contains(s))
        return Array();
